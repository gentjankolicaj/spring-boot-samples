...
public class PersistConfig {
  ...
  public StateMachine<PersistState, PersistEvent> createStateMachine() throws Exception {
    ...
    builder.configureTransitions()
        .withExternal().source(PersistState.PLACED).target(PersistState.PROCESSING)
        .event(PersistEvent.PROCESS)
        .and()
        .withExternal().source(PersistState.PROCESSING).target(PersistState.DELIVERED)
        .event(PersistEvent.SEND)
        .and()
        .withExternal().source(PersistState.DELIVERED).target(PersistState.SENT)
        .event(PersistEvent.DELIVER);
