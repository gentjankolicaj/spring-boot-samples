...
public class TasksConfig {
  ...
  public StateMachine<TasksState, TasksEvent> createStateMachine() throws Exception {
    ...
    builder.configureTransitions()
            .withExternal()
            .source(TasksState.READY).target(TasksState.FORK)
            .event(TasksEvent.FIX)
            .and()
            .withFork()
            .source(TasksState.FORK).target(TasksState.TASKS)
            .and()
            .withExternal()
            .source(TasksState.T1).target(TasksState.T1)
            .and()
            .withExternal()
            .source(TasksState.T2).target(TasksState.T2)
            .and()
            .withExternal()
            .source(TasksState.T3).target(TasksState.T3)
            .and()
            .withJoin()
