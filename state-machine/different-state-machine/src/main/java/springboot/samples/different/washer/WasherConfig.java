...
public class WasherConfig {
    ...
    public StateMachine<WasherState, WasherEvent> createStateMachine() throws Exception {
        ...
        builder.configureTransitions()
                ...
                .and()
                .withExternal().source(WasherState.WASHING).target(WasherState.RINSING).event(WasherEvent.RINSE)
                .and()
                .withExternal().source(WasherState.RINSING).target(WasherState.DRYING).event(WasherEvent.DRY)
                .and()
                .withExternal().source(WasherState.RUNNING).target(WasherState.POWER_OFF).event(WasherEvent.CUT_POWER)
                .and()
                .withExternal().source(WasherState.POWER_OFF).target(WasherState.HISTORY).event(WasherEvent.RESTORE_POWER);
